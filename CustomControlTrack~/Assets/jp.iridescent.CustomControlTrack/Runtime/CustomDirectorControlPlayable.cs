...
namespace Iridescent.Timeline
{
    ...
    /// </remarks>
    public class CustomDirectorControlPlayable : PlayableBehaviour
    {
        /// <summary>
        ...
        private bool m_stopOnPauseEvent = false;
        ...
        /// <param name="stopOnPauseEvent">Flag whether to Stop on pause</param>
        /// <returns>Returns a Playable with a DirectorControlPlayable attached</returns>
        public static ScriptPlayable<CustomDirectorControlPlayable> Create(PlayableGraph graph,
            PlayableDirector director, bool stopOnPauseEvent)
        {
            if (director == null)
                return ScriptPlayable<CustomDirectorControlPlayable>.Null;

            var handle = ScriptPlayable<CustomDirectorControlPlayable>.Create(graph);
            handle.GetBehaviour().director = director;
            handle.GetBehaviour().m_stopOnPauseEvent = stopOnPauseEvent;
        ...
        public override void OnBehaviourPause(Playable playable, FrameData info)
        {
            if (director != null && director.playableAsset != null)
            {
                ...
                {
                    if (m_stopOnPauseEvent)
                    {
                        director.Stop();
                    }
                    else
                    {
                        // ControlTrackの継ぎ目のパカつきを防ぐためにはPauseにする必要がある
                        director.Pause();
                    }
                }
            }
        ...
        {
            // リフレクション
            var tPrefabUtility = typeof(UnityEditor.PrefabUtility);
            var getRootGameObject =
                tPrefabUtility.GetMethod("GetRootGameObject", BindingFlags.Static | BindingFlags.Public);

            // When the prefab asset is updated, we rebuild the graph to reflect the changes in editor
            if ((GameObject) getRootGameObject.Invoke(null, new object[] {director}) == go)
        ...
        {
            return Math.Abs(playable.GetTime() - playable.GetPreviousTime() - info.deltaTime * info.effectiveSpeed) >
                   DiscreteTime.tickValue; // deltaTime, effectiveSpeedをfloat版に変更
        }
            ...
            if (!Mathf.Approximately((float) expectedTime, (float) director.time))
            {
                ...
                if (UnityEditor.Unsupported.IsDeveloperBuild())
                    Debug.LogWarningFormat(
                        "Internal Warning - Control track desync detected on {2} ({0:F10} vs {1:F10} with delta {3:F10}). Time will be resynchronized. Known to happen with nested control tracks",
                        playable.GetTime(), director.time, director.name, lastDelta);
#endif
                return true;
            }
